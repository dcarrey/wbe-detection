...
/*
	...
	//== ajouter le 19 janvier : initialisation d'une colonne supl�mentaire � 0
...
//==============================================================================
  ...
  /*int rootLeaf=-1,choix=-1;
  for(int i=1;i<=SpeciesTree.size;i++){
    printf("\n%d : %s", i,SpeciesTree.SpeciesName[i]);
    if(strcmp(SpeciesTree.SpeciesName[i],"outgroup") == 0)
       rootLeaf=i;
  }
  for(int i=1;i<=2*SpeciesTree.size-3-SpeciesTree.kt;i++){
    printf("\n%d : %ld -- %ld",i,SpeciesTree.ARETE[2*i-1],SpeciesTree.ARETE[2*i-2]);
	  if(SpeciesTree.ARETE[2*i-1] == rootLeaf || SpeciesTree.ARETE[2*i-2] == rootLeaf)
		  choix = i;
	}
  printf("\nchoix=%d,rootLeaf=%d",choix,rootLeaf);
  addRootByNoBranch(&SpeciesTree,choix);
	printf("\nRoot(%d) position : %d->%s", SpeciesTree.Root,SpeciesTree.size,SpeciesTree.SpeciesName[SpeciesTree.size]);
  rootLeaf=-1;
  choix=-1;
  for(int i=1;i<=GeneTree.size;i++){
    printf("\n%d : %s", i,GeneTree.SpeciesName[i]);
    if(strcmp(GeneTree.SpeciesName[i],"outgroup") == 0)
       rootLeaf=i;
  }
  for(int i=1;i<=2*GeneTree.size-3-GeneTree.kt;i++){
    printf("\n%d : %ld -- %ld",i,GeneTree.ARETE[2*i-1],GeneTree.ARETE[2*i-2]);
	  if(GeneTree.ARETE[2*i-1] == rootLeaf || GeneTree.ARETE[2*i-2] == rootLeaf)
		  choix = i;
	}
  printf("\nchoix=%d,rootLeaf=%d",choix,rootLeaf);
  addRootByNoBranch(&GeneTree,choix);
	printf("\nRoot(%d) position : %d->%s", GeneTree.Root,GeneTree.size,GeneTree.SpeciesName[GeneTree.size]);

	printf("\nTaille arbre de langues : %d",SpeciesTree.size);
	printf("\nTaille arbre de mots : %d",GeneTree.size);
*/
	for(int i=1;i<=SpeciesTree.size;i++){
		printf("\n%d : %s", i,SpeciesTree.SpeciesName[i]);
		if(strcmp(SpeciesTree.SpeciesName[i],"Root") == 0)
...
//  printf("\nNombre de feuilles : %d",SpeciesTree.size);
 ...
 // for(j=1;j<=SpeciesTree.size-1;j++){
  ...
  for(i=1;i<=SpeciesTree.size-1;i++){
        ...
        printf("--> %s ", SpeciesTree.SpeciesName[j]);
  ...
  //printf("\navgdiffblock = %lf", param.avgdiffblock);
	...
	for(i=SpeciesTree.size+1;i<=2*SpeciesTree.size-2-SpeciesTree.kt;i++){
		...
		for(j=1;j<=DTSpecies[i].nbSommet && (DTSpecies[i].nbSommet <= SpeciesTree.size-2) ;j++){
			//printf("\n(%d) ",DTSpecies[i].Tableau[j]);
			for(k=j+1;k<=DTSpecies[i].nbSommet  && (DTSpecies[i].nbSommet <= SpeciesTree.size-2);k++){
  ...
  printf("\n\nBlocked groups : %d\n",nbGroupesBloques);
	for(i=0;i<nbGroupesBloques;i++){
		printf("\n(noGroup = %d - ",groupesBloques[i][0]);
		for(j=1;j<=groupesBloques[i][0];j++){
			printf("%d ",groupesBloques[i][j]);
		}
		printf(" (subgroup of group %d)",tabGroupeA[i]);
	}
...
//				if(strcmp(param.version,"consol")==0)
				...
				//	printf("\navant expand");
					...
					/*
					printf("avgDiff=%lf / %lf",avgDiff,param.avgdiff);
          ...
          for(int m=1;m<=bestHGT[cpt_hgt].listSource[0];m++){
            ...
            if(strcmp(SpeciesTree.SpeciesName[bestHGT[cpt_hgt].listDestination[m]],"outgroup") == 0)
              outgroupFound = true;
          ...
          //=========================================================================================
          //== Calcul de la probabilit� d'avoir un transfert
          //=========================================================================================
          ...
          //printf("\n=====> ");
            ...
            //printf("nbSimilaire = %d",nbSimilaire);
						...
						if((nbSimilaire > 0) &&(nbSimilaire1 > 0))
							temoinY = 1; // 1 = il y a des �l�ments du m�me groupe � la source et a la destination
					}
          //printf("\n==>temoinZ=%d, temoinY=%d", temoinZ, temoinY);
					if((temoinZ == 1) && (temoinY == 0)){
						printf("\nGroupe a ne pas briser");
						bestHGT[cpt_hgt].valide = 0;
						continue;
					}
