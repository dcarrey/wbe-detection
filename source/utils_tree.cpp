...
void loadAdjacenceMatrix( double **Adjacence, long int *ARETE, double *LONGUEUR,int size,int kt){
	...
	/*for(int i=1;i<=2*size-2;i++){
		...
		for(int j=1;j<=2*size-2;j++){
			if(Adjacence[i][j] == INFINI) printf("");
			else printf(" %d",j,Adjacence[i][j]);
		}
	}
	printf("\n");*/
}
...
//
//===============================================================================================================================
void RechercherBipartitionAvecRacine(long int *ARETE,double ** DIST,double ** Adjacence2,struct DescTree *DT,const int n,int kt,int root)
{
	/*/===== variables =====*/
	int i,j,k,l,taille,plus,ns=2*n-2;
	double ** Adjacence =(double**)malloc((2*n+1)*sizeof(double*));
...
int lectureNewick(const char * newick, long int * ARETE, double * LONGUEUR, char ** lesNoms, int *kt)
{
	...
	/*printf("\nDegre des noeuds internes (nb especes=%d)",n);
	for(i=1;i<=2*n-3;i++){
		printf("\n%d-->%d",i,tableau[i]);
	}*/
...
//== ajouter la martice de gene dans le fichier input
